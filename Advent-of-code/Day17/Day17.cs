...
public class Day17
{
    ...
    private int RunLogic(IReadOnlyList<List<Position>> data)
    {
        ...
        while (frontier.Count != 0)
        {
            ...
            var neighbors = FindNeighbors(data, current, 1);
    ...
    private List<Position> FindNeighbors(IReadOnlyList<List<Position>> data, Position current, int stepSize)
    {
        var neighbors = new List<Position>();
        if (current.Row > 1-stepSize)
        {
            neighbors.Add(data[current.Row-stepSize][current.Col]);
        }
        if (current.Col > 1-stepSize)
        {
            neighbors.Add(data[current.Row][current.Col-stepSize]);
        }
        if (current.Row < data.Count-stepSize)
        {
            neighbors.Add(data[current.Row+stepSize][current.Col]);
        }
        if (current.Col < data[0].Count-stepSize)
        {
            neighbors.Add(data[current.Row][current.Col+stepSize]);
        }
